/*
	...
	Host-side "board" for the atmega4809 modern-AVR blink demo. It loads the
	firmware, watches PA0 toggle through the modern PORT model (which reuses the
	classic IOPORT IRQ mesh, so the hook below is identical to a classic board),
	records PA0 to a VCD file, and runs for a bounded number of simulated
	milliseconds so the demo terminates on its own.
...
{
	elf_firmware_t f = {{0}};
	const char * fname = "atmega4809_blink.axf";
	...
	avr_vcd_init(avr, "atmega4809_blink.vcd", &vcd_file, 1000 /* usec */);
	...
	printf("\nDone: PA0 toggled %lu times in 200 ms; trace in atmega4809_blink.vcd\n",
		pa0_toggles);
	return 0;
}
